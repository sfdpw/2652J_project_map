function tracking_table_generator_sw_laterals() {
  ...
  for (const sw_lateral of json_200_laterals_2["features"])

  {
   ...
   sw_lateral_row[1] = "<a href=\"..\\index.html#20/" + 
                     ...
                     "\" target=\"_blank\">" +
                     sw_lateral.properties.MAXIMO_ID + "</a>";
   ...
   for (var bi in sw_lateral.properties.PP_HISTORY)
    ...
    for (bi = 1; bi < sw_lateral_row[6].length; bi++ )
    
      {
       ...
       while (base_sov[NN]['Bid Item'] != sw_lateral_row[6][bi].replace('SW-0','SW-')) { NN++; }
  ...
  // status sorting        
   ...
   if ( 
       sw_lateral_row[4].includes('Install (N)') ||
       sw_lateral_row[4] == 'Replace (E) with (N)' || 
       sw_lateral_row[4] == 'Extend and Connect (E) or Replace with (N) as Directed' ||        
       sw_lateral_row[4] == 'Replace (E) with (N), Extend and Connect' ||
       sw_lateral_row[4] == 'Intercept to Connect (E) or Replace with (N) as Directed' ||
       sw_lateral_row[4] == 'Line (E) with CIPL' 
       )
   ...
   else if (sw_lateral_row[4].includes('Replace (E) with (N) as Directed') && sw_lateral_row[9] != 'Do Not Replace')
    ...
    
      {
          ...
          if (sw_lateral_row[12] == 'MCN' || sw_lateral_row[12] == 'NET' )
       ...
       
            {
             ...
             if (sw_lateral_row[6][0].includes('SW-37'))
   ...
   
   {
    ...
    if      (sw_lateral_table[rr][0] == 'A' || 
             ...
             sw_lateral_table[rr][0] == 'L' ) {row_shade='no_action';}
